...
Humans = ["basketball", "bolt1", "diver", "girl", "graduate", "gymnastics1", "gymnastics2", "gymnastics3", "handball1", "handball2", "iceskater1", "iceskater2", "marathon", "matrix", "polo", "rowing", "shaking", "singer2", "singer3", "soccer1", "soccer2", "soldier", "surfing"]
Vehicles = ["car1", "wiper"]
Animals = ["agility", "animal", "ants1", "birds1", "birds2", "butterfly", "crabs1", "fernando", "fish1", "fish2", "flamingo1", "kangaroo", "lamb", "monkey", "nature", "rabbit", "rabbit2", "snake", "tiger", "zebrafish"]
...
def generate_combined_csv(csv_files, output_file):
    ...
    for fn, l in zip(categories, data_lists):
        ...
        metrics = {"category": [], "IoU": [], "find_rates": [], "acc": [], "miou": [], "dice": []}
        ...
        for file_name in csv_files:
            ...
            metrics["IoU"].append(float(file_name.split('_')[-1].replace('.csv', '')) / 10.)
            metrics["find_rates"].append((df["find_rates"] * df["total_images"]).sum() / total_images)
            metrics["acc"].append((df["acc"] * df["total_images"]).sum() / total_images)
            metrics["miou"].append((df["miou"] * df["total_images"]).sum() / total_images)
            metrics["dice"].append((df["dice"] * df["total_images"]).sum() / total_images)
...
csv_files = [f"/home/dogus/final_ws/solov2_venv/src/AdelaiDet/results_0_{str(x)}.csv" for x in range(1,10)]
