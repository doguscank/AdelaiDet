...
def draw_bboxes(image_path, gt_bboxes, pred_bboxes):
    ...
    for bbox in gt_bboxes:
        ...
        cv2.rectangle(image, (x - w//2, y - h//2), (x + w//2, y + h//2), colors['gt'], 2)
        cv2.putText(image, class_names[class_id], (x + w//2, y - h//2 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, colors['gt'], 2)

    # Draw the predicted bounding boxes
    for bbox in pred_bboxes:
        x, y, w, h, class_id = bbox
        x, y, w, h = int(x * image.shape[1]), int(y * image.shape[0]), int(w * image.shape[1]), int(h * image.shape[0])
        cv2.rectangle(image, (x - w//2, y - h//2), (x + w//2, y + h//2), colors['pred'], 2)
        cv2.putText(image, class_names[class_id], (x - w//2, y - h//2 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, colors['pred'], 2)
