...
if __name__ == '__main__':            
    for pred in sorted(glob(f"/home/dogus/final_ws/solov2_venv/src/AdelaiDet/multi_size_image_results/bboxes/*.txt")):
        fn = pred.split("/")[-1]
        df = pred_to_yolo(pd.read_csv(pred, header=None))
        os.makedirs(os.path.join(os.path.expanduser("~"), "multi_size_image_results"), exist_ok=True)
        df.to_csv(os.path.join(os.path.expanduser("~"), "multi_size_image_results", fn), header=None, index=None, sep=" ")